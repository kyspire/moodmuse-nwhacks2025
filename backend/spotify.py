...
# Function to read the playlist link from the file
def get_playlist_link_from_file(filename="detected_emotions.txt"):
    try:
        with open(filename, "r") as file:
            line = file.readline().strip()  # Read the first line and remove extra spaces
            if line:
                # Extract playlist link after the emotion if format is "emotion: link"
                parts = line.split(": ")
                if len(parts) == 2:
                    return parts[1].strip()  # Return the link only
                else:
                    return line  # If the line only contains the link
    except FileNotFoundError:
        print("Error: detected_emotions.txt file not found.")
...
if user_token:
    # Read the playlist link from the detected_emotions.txt file
    playlist_link = get_playlist_link_from_file()

    if playlist_link:
        print(f"Using playlist link from file: {playlist_link}")
        playlist_id = playlist_link.split("/")[-1].split("?")[0]  # Extract playlist ID
        ...
        print("Could not read playlist link from file.")
else:
